'use client';

import { useState, useEffect } from 'react';
import Link from 'next/link';

interface SkillMeta {
  id: string;
  name: string;
  description: string;
  available: boolean;
  size: number;
  updatedAt: string | null;
}

const MASTER_PROMPT = `You are an elite TikTok football scriptwriter. I'm attaching a "Channel Profile" — a deep structural analysis of a creator's voice, hooks, narrative beats, and storytelling techniques, built from studying dozens of their highest-performing videos.

Study it closely, then write me a complete, ready-to-record script in their exact style for this topic: [YOUR TOPIC HERE]

Requirements:
- Match their voice, persona, and relationship with the viewer
- Open with one of their signature hook formulas — don't invent a generic one
- Follow their narrative structure beat-by-beat, in order — don't skip steps
- Weave in real, specific facts (matches, minutes, players, score progressions) — emotion is the DELIVERY, facts are the CONTENT. Emotion alone is empty poetry.
- Clean spoken words only — no [CAM], no [BROLL], no camera directions, no brackets
- Target length: 2–2.5 minutes read aloud naturally at a natural pace (roughly 280–350 words)

After the script, give me:
- Hook: (the opening line alone)
- Caption: (under 150 characters)
- Hashtags: (6–8 relevant tags)`;

function formatBytes(b: number) {
  return b < 1024 ? `${b} B` : `${(b / 1024).toFixed(1)} KB`;
}

function CopyButton({ text, label = 'Copy Prompt' }: { text: string; label?: string }) {
  const [copied, setCopied] = useState(false);
  return (
    <button
      onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 1800); }}
      className="cursor-target flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-white/10 transition-colors"
      style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)' }}
    >
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" opacity="0.6">
        <path strokeLinecap="round" strokeLinejoin="round" d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184" />
      </svg>
      <span className="text-[10px] font-black uppercase tracking-widest text-white/70">{copied ? 'Copied ✓' : label}</span>
    </button>
  );
}

function SkillCard({ skill }: { skill: SkillMeta }) {
  const [open, setOpen] = useState(false);

  const download = () => {
    if (!skill.available) return;
    window.location.href = `/api/football/skills/${skill.id}`;
  };

  return (
    <div
      className="rounded-2xl overflow-hidden"
      style={{ background: 'rgba(0,0,0,0.55)', border: '1px solid rgba(255,255,255,0.12)' }}
    >
      <button
        onClick={() => setOpen(o => !o)}
        className="cursor-target w-full flex items-center gap-3 px-4 sm:px-5 py-3.5 text-left hover:bg-white/5 transition-colors"
      >
        <div className="flex-1 min-w-0">
          <div className="font-black italic text-base sm:text-lg text-white leading-tight mb-1.5">
            {skill.name}
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            {skill.available ? (
              <span className="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider text-white" style={{ background: '#15803d' }}>
                ● Ready · {formatBytes(skill.size)}
              </span>
            ) : (
              <span className="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider text-white" style={{ background: '#92400E' }}>
                ⏳ Generating…
              </span>
            )}
          </div>
        </div>
        <div
          className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform duration-300"
          style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
          </svg>
        </div>
      </button>

      {open && (
        <div className="border-t px-4 sm:px-5 pt-3.5 pb-4" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
          <p className="text-[12px] text-white/65 leading-relaxed mb-3.5">{skill.description}</p>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <button
              onClick={download}
              disabled={!skill.available}
              className="cursor-target flex items-center gap-2 px-4 py-2 rounded-xl font-black uppercase tracking-widest text-[10px] text-white transition-opacity disabled:opacity-40"
              style={{ background: skill.available ? 'linear-gradient(135deg,#7c3aed,#4f46e5)' : 'rgba(255,255,255,0.08)' }}
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
              </svg>
              {skill.available ? 'Download .md' : 'Not ready yet'}
            </button>
            {skill.updatedAt && (
              <span className="text-[10px] text-white/30 font-semibold italic">
                Updated {new Date(skill.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
              </span>
            )}
          </div>
        </div>
      )}
    </div>
  );
}

export default function SkillsPage() {
  const [skills, setSkills] = useState<SkillMeta[] | null>(null);

  useEffect(() => {
    fetch('/api/football/skills').then(r => r.json()).then(d => { if (d.ok) setSkills(d.skills); }).catch(() => {});
  }, []);

  return (
    <div className="min-h-screen px-4 py-5 sm:py-7 max-w-3xl mx-auto">

      {/* Header */}
      <div className="flex items-center gap-4 mb-6 sm:mb-8">
        <div className="relative flex-shrink-0">
          <div
            className="w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden"
            style={{ border: '2px solid rgba(255,255,255,0.25)', boxShadow: '0 0 0 1px rgba(255,255,255,0.08), 0 8px 32px rgba(0,0,0,0.5)' }}
          >
            <img
              src="https://images.squarespace-cdn.com/content/v1/66e051a492185d22d4dafad3/1729342217416-U6VL0Q9QS0H4O3FDHYUE/imdiez.png"
              alt="Diez"
              className="w-full h-full object-cover"
            />
          </div>
          <span className="absolute bottom-0.5 right-0.5 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-emerald-400 border-2 border-white animate-pulse-dot" />
        </div>
        <div>
          <div
            className="text-[10px] font-black uppercase tracking-[0.25em] mb-1"
            style={{ color: 'rgba(255,255,255,0.75)', textShadow: '0 1px 6px rgba(0,0,0,0.95), 0 2px 12px rgba(0,0,0,0.8)' }}
          >
            Narrative Skill Files
          </div>
          <h1
            className="font-black italic text-2xl sm:text-4xl text-white leading-none mb-1"
            style={{ textShadow: '0 2px 8px rgba(0,0,0,0.9), 0 4px 24px rgba(0,0,0,0.6)' }}
          >
            STORYTELLING SKILLS
          </h1>
          <p
            className="text-xs sm:text-sm font-semibold"
            style={{ color: 'rgba(255,255,255,0.75)', textShadow: '0 1px 6px rgba(0,0,0,0.95), 0 2px 12px rgba(0,0,0,0.8)' }}
          >
            Download voice profiles, drop them into Claude — write in any creator&apos;s style.
          </p>
        </div>
      </div>

      {/* How it works */}
      <div className="mb-6 sm:mb-8 rounded-2xl overflow-hidden" style={{ background: 'rgba(0,0,0,0.55)', border: '1px solid rgba(255,255,255,0.12)' }}>
        <div className="px-4 sm:px-5 pt-4 pb-2">
          <div className="flex items-center gap-2.5 mb-1">
            <span className="text-xl">🧠</span>
            <h2 className="font-black italic text-lg sm:text-xl text-white uppercase tracking-tight" style={{ textShadow: '0 1px 8px rgba(0,0,0,0.9)' }}>
              How To Use These
            </h2>
          </div>
          <p className="text-[12px] text-white/55 leading-relaxed">
            Each skill file is a deep structural analysis of one creator&apos;s voice — their hooks, narrative beats, rhetorical devices, and writing formula — distilled from studying dozens of their best-performing videos. Attach one to a Claude conversation and it can write completely new scripts in that exact style.
          </p>
        </div>

        <div className="px-4 sm:px-5 pb-4">
          <ol className="space-y-2 mb-4">
            {[
              'Download the skill file (.md) for the creator style you want.',
              'Open Claude Desktop → start a new chat (or add it to a Project\'s Knowledge so every chat in that project can use it).',
              'Attach the downloaded .md file to your message (📎 upload file).',
              'Paste the prompt below — swap in your topic — and hit send.',
            ].map((step, i) => (
              <li key={i} className="flex items-start gap-3">
                <span className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black text-white mt-0.5"
                  style={{ background: 'linear-gradient(135deg,#7c3aed,#4f46e5)' }}>{i + 1}</span>
                <span className="text-[12px] text-white/70 leading-relaxed">{step}</span>
              </li>
            ))}
          </ol>

          <div className="rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
            <div className="flex items-center justify-between px-3.5 py-2.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Ready-to-paste prompt</span>
              <CopyButton text={MASTER_PROMPT} />
            </div>
            <pre className="px-3.5 py-3 text-[11px] text-white/65 leading-relaxed whitespace-pre-wrap font-mono overflow-x-auto">{MASTER_PROMPT}</pre>
          </div>
        </div>
      </div>

      {/* Skills list */}
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center gap-2.5 mb-3">
          <span className="text-xl">📂</span>
          <h2 className="font-black italic text-xl sm:text-2xl text-white uppercase tracking-tight" style={{ textShadow: '0 1px 8px rgba(0,0,0,0.9), 0 2px 20px rgba(0,0,0,0.6)' }}>
            Available Skills
          </h2>
        </div>
        <div className="flex flex-col gap-2.5">
          {skills === null ? (
            <p className="text-[11px] text-white/30 italic px-1">Loading…</p>
          ) : (
            skills.map(s => <SkillCard key={s.id} skill={s} />)
          )}
        </div>
      </div>

      {/* Back to media kit */}
      <Link
        href="/"
        className="cursor-target w-full flex items-center justify-between px-5 py-3 rounded-xl mt-2 hover:opacity-90 transition-opacity"
        style={{ background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(255,255,255,0.15)' }}
      >
        <span className="text-xs sm:text-sm font-black italic text-white">← Back to media kit</span>
      </Link>
    </div>
  );
}
